...
describe('ReactiveList<ListItem>', () => {
  ...
  const findSelectedLength = (vm: ReactiveListVM<ListItem>) =>
    findSelected(vm).length;
  ...
  beforeEach(() => {
    ...
    vmChanges = [];
    subscription = selectable.vm$.subscribe((s) =>
      vmChanges.push(s.status.value)
    );
  });
  ...
  describe('initialization', () => {
    it('should have public properties', () => {
      ...
      expect(selectable).toHaveObservables([
        'list$',
        'selected$',
        'status$',
        'vm$',
      ]);
      expect(selectable).toHaveMethods([
        'addItems',
        'selectItems',
        'selectItemsById',
        'selectAll',
  ...
  describe('addItems', () => {
    ...
    it('should list the items added', () => {
      const findItemIds = (vm: ReactiveListVM<ListItem>) =>
        findList(vm).map((item) => item.id);
      // Add 2 items
